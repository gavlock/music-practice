...
class Note {
	constructor(nameOrIndex) {
		...
		this.staffIndex = Math.floor(
			(this.noteIndex // based on C0
			 + Math.floor((this.noteIndex - 1) / 12) // adjust for the missing semitone between B and C
			 + Math.floor((this.noteIndex - 6) / 12) // adjust for the missing semitone between E and F
			) /2); // divided by 2 because staff lines deal in tones, not semitones
	}
...
function staffIndexForNote(note, bottomLine) {
	return note.staffIndex - bottomLine.staffIndex;
}
...
class Staff {
	...
	addNote(note) {
		this.notes.push(noteFromNoteOrName(note));
	}
