...
function getIndexFromFrequency(frequency) {
	const relativeIndex = Math.round((12 * Math.log(frequency / concertAFrequency) / Math.log(2)));
	return concertANoteIndex + relativeIndex;
}
...
Note.indexFromFrequency = (frequency) => {
	return getIndexFromFrequency(frequency);
};

Note.FromFrequency = (frequency) => {
	const noteIndex = getIndexFromFrequency(frequency);
