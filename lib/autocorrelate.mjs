export default class Autocorrelate {
	...
	calcOneCorrelation(lag) {
		let sum = 0;
		for (let t = 0 ; t < this._dataLength - lag ; ++t)
			sum += this._get(t) * this._get(t + lag);
		return sum;
	}
	...
	calcCorrelation(minLag = 0, maxLag = this._maxPossibleLag) {
		...
		for (let lag = minLag ; lag <= maxLag ; ++lag)
			correlation[lag - minLag] = this.calcOneCorrelation(lag);

		const zeroLagCorrelation = (minLag == 0) ? correlation[0] : this.calcOneCorrelation(0);
		const scale = zeroLagCorrelation ? (0.5 / zeroLagCorrelation) : 1;
